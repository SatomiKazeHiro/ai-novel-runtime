...
- [x] **Q23** (2026-07-04): v2-architecture §6.6/§7.1/§10 同步 Q15-Q22 已落实 — commit `7d81ae1`
- [x] **Q24** (2026-07-04): V2 Chapter Design 布局样式整理 — commit `fbc22df`，5 步骤条（horizontal cap-step-bar）+ 归档 stepper + 补 30+ class CSS（cap-card/cap-eyebrow/cap-body-sm/split-row 等）；Q24 调研期间试过 4 方案（侧栏 / 卡片内 5 圆点 / step rail / 右侧 sticky anchor 锚点）→ 最终保留最简的 horizontal cap-step-bar 方案
- [x] **Q24.1** (2026-07-04): V2Step4Panel 分析失败态加内联"重新生成"按钮 — commit `a104c95`，1 文件 9 行；5 个 analyzer tab 共用的失败 UI 之前只显示错误文本，底部"重新生成"按钮埋太深，失败时无法快速 retry；内联按钮 + `:loading="regenerating === item.key"` + `:disabled="analyzeRunning"` 与底栏按钮行为一致
- [x] **Q25** (2026-07-04): V2 Chapter Design 视觉精简 (editorial minimal 调优) — commit `e468054`，3 文件 91 行；frontend-design skill refined minimal 方向, 4 处: cap-step-bar 当前 step 升级 (24→28px + 4px tint 光晕 + 编号改 mono) + future 段 connector dashed + 完成段 terracotta; design-flow > cap-card 间距 24px (替代 4px 糊一长条); Step 3 Prompt 预览 + Step 5 analyzer pre-analyze 空态改中文 dashed border 卡片 (避免 Naive UI n-empty 英文 placeholder); analyzer-idle 同步升级 dashed + non-italic。不动结构 / 不加新色 / 不加动效 / 全程复用 base.css 现有 token
- [x] **Q26** (2026-07-04): V2 timeline-extractor prompt 改写解决"事件过多+重复描述同一核心事件" — 1 文件 `apps/server/src/services-v2/timeline-extractor.ts` prompt 重构。**根因**：旧 prompt "按叙事顺序提取所有事件" + 无运行时去重 → AI 把同一核心事件的不同侧面（许青入门 / 拜师 / 入宗门）拆成多条。**对比 V1**：`services/memory-extractor.ts` 有 Jaccard 0.7 运行时聚类兜底 + shared `extract-prompt.ts` 有 7/6/5/4 分评分 + 主角参与+8=主要事件分流；V2 两层都没了。**改写（方案 A：仅改 prompt，不加运行时候选 B 兜底）**：
  1. system message 改为原则导向（笔墨浓重/推动剧情/情感强烈优先 + 合并同一核心事件 + 排除一笔带过/景物/心理/过渡 + 一章 3–5 条上限）
  2. user message 在字段说明后加【去重与精炼】4 条硬约束（去重合并 / 数量上限 / 排除项 / importance 重定义 major 占大半篇幅）
  3. summary 从 1-2 句话改为 "一句话精炼概括核心事件, 不超过 30 字"
  4. 保留 importance 字段（major/normal/minor）让 AI 按新标准重新评分，schema 不变
  **不动**：字段定义、validate 函数、importance 枚举、defaultAnchorName、Jaccard 兜底（用户测完发现不奏效时再加 = 方案 B）。
- [x] **Q27-1** (2026-07-04): V2Timeline.vue 只读版重写用上 schema 已建字段 — 1 文件 `apps/web/src/views-v2/V2Timeline.vue` 全部重写（70 → 290 行）。**调研发现**：schema 已建 `V2TimelineAnchor + V2TimelineEvent + V2TimelineRelation` 三表（[schema.prisma:567-616]），archive 流程 [chapters-archive.ts:180-206] 自动写数据，但前端只读 list 连 schema 的 `title/summary/importance/timeExpression/participants/location` 6 字段全没用上。**改动**：
  1. **anchor 切换 chip 工具栏**：anchor > 1 时显示，cap-chip token 复用 + `is-active` terracotta tint 状态。
  2. **章节分组**：事件按 `chapterNumber` 升序分组（先按 anchor 再按章节号），每组一个 `cap-card` 标题栏（eyebrow "第 N 章" + 章节标题 + "N 条" 计数）。
  3. **事件卡片**：24px mono 编号徽章 + 标题 / summary（≤30 字 AI 已限）/ meta 行（timeExpression `raw` mono 蓝字 / location / participants，dot 分隔）/ `importance` badge（major terracotta / normal blueprint / minor sage）。
  4. **空态**：撤销 Naive UI n-empty 英文占位 → 中文章案 + 归档引导。
  5. **page-head lede**：动态显示"X 条事件 · 跨 N 章 · M 个锚点"。
  **API**：复用 `v2TimelineApi.list`（已含 events include）+ 新增 `v2ChaptersApi.list` 一次拉取章节表给"第 N 章 · 标题"显示用。**Not done**：UI CRUD（V2 是 "AI 自动写、人工查阅" 哲学 + 与 archive 自动写一致性难维护，**V2 暂不做**）；V2TimelineRelation 表可视化（表已留口但 extractor 没抽这层先不动）。
  **not changed**：schema 不动（多 anchor + 富字段 + relation 设计本身合理），后端不动。
