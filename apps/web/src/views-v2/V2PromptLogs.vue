  ...
  <div>
    <header class="page-head">
      <div class="page-head__text">
        <span class="cap-eyebrow is-accent">V2 · PROMPT LOGS</span>
        <h1 class="page-head__title">调用日志</h1>
        <p class="page-head__lede cap-body-sm">所有 AI 调用的完整记录 — prompt、token 用量、响应时间、错误追踪。</p>
      </div>
    ...
    <n-space vertical size="large">
      <div class="metric-board">
        ...
        <div class="metric-card">
          ...
          <span class="metric-card__delta cap-caption metric-card__delta--positive">{{ successRateLabel }}</span>
        </div>
        <div class="metric-card metric-card--error">
          ...
          <span class="metric-card__delta cap-caption metric-card__delta--error">{{ errorRateLabel }}</span>
        </div>
      </div>
      ...
      <n-space vertical v-if="detailLog">
        <n-descriptions bordered :column="3" size="small">
          ...
          <n-descriptions-item label="类型">
            <span class="prompt-chip" :data-tone="detailCallType.tone">{{ detailCallType.label }}</span>
          </n-descriptions-item>
          <n-descriptions-item label="状态">
            <span class="prompt-chip" :data-tone="detailCallStatus.tone">{{ detailCallStatus.label }}</span>
          </n-descriptions-item>
...
import { v2PromptLogApi } from '../api-v2/prompt-logs'
...
const columns: DataTableColumns<any> = [
  ...
  {
    title: '类型', key: 'callType', width: 110,
    render: (row) => {
  ...
  { title: '预估 Token', key: 'estimatedTokens', width: 110, align: 'right' },
  {
    title: '实际 Token', key: 'totalTokens', width: 130, align: 'right',
    render: (row) => {
      ...
      return h('div', { class: 'token-cell' }, [
        ...
        diff !== 0
          ? h('span', { class: `token-cell__delta token-cell__delta--${color}` }, `(${diff > 0 ? '+' : ''}${formatNumber(diff)})`)
          : null
      ])
    }
  },
...
async function fetchLogs() {
  ...
  loading.value = true
  try {
    const res = await v2PromptLogApi.list(storyId.value, {
      callType: filterCallType.value || undefined,
      page: page.value,
...
@media (max-width: 880px) { .metric-board { grid-template-columns: repeat(2, 1fr); } }
@media (max-width: 480px) { .metric-board { grid-template-columns: 1fr; } }
...
.metric-card:hover { border-color: var(--color-mid-gray); }
.metric-card--error { border-left: 2px solid var(--color-error); }

.metric-card__label { color: var(--text-tertiary); }
.metric-card__num {
...
.metric-card--error .metric-card__num { color: var(--color-error); }
.metric-card__delta { font-variant-numeric: tabular-nums; }
...
.token-cell__num { font-variant-numeric: tabular-nums; }
.token-cell__delta {
  font-family: var(--font-mono);
  font-size: 11px;
  font-variant-numeric: tabular-nums;
