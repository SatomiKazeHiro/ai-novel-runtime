  ...
  <div class="reader">
    ...
    <aside class="reader__aside">
      <div class="reader__aside-head">
        <span class="cap-eyebrow">ARCHIVED</span>
          ...
          <article v-if="currentChapter" :key="currentChapter.id" class="reader__article">
            <header class="reader__article-head">
              <div class="reader__article-meta">
                ...
                <span v-if="currentChapter.isSideStory" class="reader__article-tag">番外</span>
              </div>
              <h1 class="reader__article-title">{{ currentChapter.title }}</h1>
              <div v-if="currentChapter.summary" class="reader__article-summary">
...
const fullTree = ref<ChapterNode[]>([])
const currentChapter = ref<ChapterNode | null>(null)
const selectedId = ref('')
const loading = ref(false)

/** 展平并只保留 archived 状态, 按 number 升序. */
const archivedChapters = computed(() => {
  const out: ChapterNode[] = []
  function walk(nodes: ChapterNode[]) {
    for (const n of nodes) {
      if (n.status === 'archived') out.push(n)
...
async function loadTree() {
  ...
  loading.value = true
  try {
    const res = await chaptersApi.getTree(storyId.value)
    fullTree.value = (res.data?.data || []) as ChapterNode[]
...
async function selectChapter(id: string) {
  ...
  try {
    ...
    currentChapter.value = data as ChapterNode
  } catch {
    currentChapter.value = null
  }
}
...
void getChapterStatus // 保留 import 树稳定, 未来章节状态徽章可放回阅读头
</script>
