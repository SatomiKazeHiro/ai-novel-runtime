  ...
  <div>
    ...
    <div v-if="!editor.editMode">
      <n-space justify="space-between" align="center" style="margin-bottom: 16px">
        <n-h1>章节工作台</n-h1>
        <n-button type="primary" @click="tree.openCreateRoot">新建根章节</n-button>
      </n-space>
      ...
      <n-empty v-else-if="tree.chapterTree.length === 0" description="暂无章节，点击新建根章节开始创作" />
    ...
    <div v-else>
      ...
      <n-card title="Step 1：配置区" style="margin-bottom: 24px">
        ...
        <!-- 剧情弧线 -->
        <n-grid v-if="editor.plotArcs.length > 0" :cols="3" :x-gap="12" :y-gap="12" style="margin-bottom: 16px">
          <n-grid-item v-for="arc in editor.plotArcs" :key="arc.id">
      ...
      <n-card v-if="!isReadonly" title="Step 2：生成区" style="margin-bottom: 24px">
        ...
        <n-space style="margin-bottom: 12px">
          <n-button type="primary" @click="handleGeneratePrompt" :loading="prompt.previewLoading">生成 Prompt</n-button>
          <n-button @click="prompt.copyPrompt" :disabled="!prompt.editablePrompt">复制 Prompt</n-button>
        </n-space>

        <!-- Prompt 预算 -->
        <n-card v-if="prompt.tokenStats" size="small" style="margin-bottom: 12px" :bordered="false">
          <n-space vertical>
            <n-space justify="space-between" align="center">
              <n-text strong>Prompt 预算</n-text>
              <n-space>
                <n-tag size="small" :type="prompt.tokenStats.totalTokens > prompt.DANGER_THRESHOLD ? 'error' : prompt.tokenStats.totalTokens > prompt.WARN_THRESHOLD ? 'warning' : 'success'">
                  {{ prompt.tokenStats.totalTokens.toLocaleString() }} / {{ prompt.MODEL_MAX_TOKENS.toLocaleString() }} tokens
                </n-tag>
                <n-tag v-if="prompt.layerStats.some((l: any) => l.truncated)" size="small" type="error">⚠️ 有层被截断</n-tag>
              </n-space>
            </n-space>
            <n-progress
              :percentage="Math.min(100, Math.round((prompt.tokenStats.totalTokens / prompt.MODEL_MAX_TOKENS) * 100))"
              :status="prompt.tokenStats.totalTokens > prompt.CRITICAL_THRESHOLD ? 'error' : prompt.tokenStats.totalTokens > prompt.WARN_THRESHOLD ? 'warning' : 'success'"
        ...
        <n-space align="center" style="margin-bottom: 16px">
          <n-button type="primary" @click="handleGenerateDefault" :loading="drafts.generating" :disabled="drafts.generating">
            {{ drafts.generating ? '生成中...' : '生成默认候选 (×3)' }}
          </n-button>
          <n-button @click="drafts.showCustomModal = true" :disabled="drafts.generating">生成自定义候选 (×1)</n-button>
          <n-text v-if="drafts.generating" depth="3">AI 正在创作中，请耐心等待</n-text>
        </n-space>

        <!-- 候选展示 -->
        <n-grid v-if="drafts.drafts.length > 0" :cols="3" :x-gap="12" :y-gap="12">
            ...
            <n-card size="small" style="display: flex; flex-direction: column" :bordered="true">
              ...
              <n-tabs type="segment" size="small" style="flex: 1">
                ...
                <n-tab-pane name="prompt" tab="Prompt">
                  <n-scrollbar style="max-height: 280px">
                    <n-p style="white-space: pre-wrap; font-size: 12px; color: #666">{{ prompt.formatCompiledPrompt(draft.compiledPrompt) }}</n-p>
                  </n-scrollbar>
                </n-tab-pane>
              ...
              <template #footer>
                <n-space align="center">
                  <n-button size="small" type="primary" @click="handleSelectDraft(draft.id)" :disabled="draft.status === 'generating' || !draft.content">采用</n-button>
                  <n-button size="small" @click="drafts.scoreDraft(draft.id)" :loading="drafts.scoringDraftId === draft.id" :disabled="!draft.content || (drafts.scoringDraftId !== null && drafts.scoringDraftId !== draft.id)">评分</n-button>
                  <n-button size="small" @click="drafts.confirmDeleteDraft(draft.id, draft.version)">删除</n-button>
                  <n-text v-if="draft.content" depth="3" style="font-size: 12px">{{ draft.content.length.toLocaleString() }} 字</n-text>
                </n-space>
              </template>
            </n-card>
          </n-grid-item>
      ...
      <n-card title="Step 3：正文与归档" v-if="!isReadonly && drafts.drafts.length > 0 && drafts.drafts.some((d: any) => d.status === 'completed' || d.status === 'selected')">
        ...
        <n-space align="center" justify="space-between">
          <n-space>
            <n-button type="primary" @click="editor.saveContent">保存正文</n-button>
            <n-button @click="handleArchive" :loading="archiving" :disabled="!editor.editForm.content?.trim() || archiving">
              归档{{ !editor.editForm.content?.trim() ? '（需先填写正文）' : '' }}
            </n-button>
          </n-space>
    ...
    <!-- ========== 弹窗 ========== -->
    <!-- 新建根章节 -->
    <n-modal v-model:show="tree.showCreateModal" title="新建根章节" preset="card" style="width: 500px">
      <n-form :model="tree.createForm" label-placement="left" label-width="80">
        <n-form-item label="标题" required>
          <n-input v-model:value="tree.createForm.title" placeholder="章节标题" />
        </n-form-item>
        <n-form-item>
          <n-checkbox v-model:checked="tree.createForm.isSideStory">番外章节</n-checkbox>
        </n-form-item>
        <n-form-item label="大纲">
          <n-input v-model:value="tree.createForm.outline" type="textarea" placeholder="章节大纲" />
        </n-form-item>
      </n-form>
      <template #footer>
        <n-space justify="end">
          <n-button @click="tree.showCreateModal = false">取消</n-button>
    ...
    <!-- 发展 -->
    <n-modal v-model:show="tree.showDevelopModal" title="发展下一章" preset="card" style="width: 500px">
      ...
      <template #footer>
        <n-space justify="end">
          <n-button @click="tree.showDevelopModal = false">取消</n-button>
          <n-button type="primary" @click="handleDevelop" :disabled="!tree.developForm.title.trim()">创建</n-button>
        </n-space>
      </template>
    </n-modal>

    <!-- 自定义候选 -->
    <n-modal v-model:show="drafts.showCustomModal" title="自定义候选生成" preset="card" style="width: 400px">
      <n-form label-placement="left" label-width="100">
        <n-form-item label="Temperature">
          <n-slider v-model:value="drafts.customTemp" :min="0" :max="2" :step="0.05" />
          <n-text>{{ drafts.customTemp.toFixed(2) }}</n-text>
        </n-form-item>
        <n-form-item label="Max Tokens">
          <n-input-number v-model:value="drafts.customMaxTokens" :min="512" :max="8192" :step="256" />
        </n-form-item>
      </n-form>
      <template #footer>
        <n-space justify="end">
          <n-button @click="drafts.showCustomModal = false">取消</n-button>
          <n-button type="primary" @click="handleGenerateCustom" :loading="drafts.generating">生成</n-button>
        </n-space>
      </template>
    </n-modal>

    <!-- 评分结果 -->
    <n-modal v-model:show="drafts.showScoreModal" title="评分结果" preset="card" style="width: 520px">
      <n-space v-if="drafts.scoreResult" vertical size="large">
        <n-space justify="center" align="center" vertical style="padding: 8px 0">
          <n-text style="font-size: 48px; font-weight: 700; color: #1890ff">{{ drafts.scoreResult.totalScore }}</n-text>
        </n-space>
        <n-divider />
        <n-space vertical size="small">
          <n-space v-for="(label, key) in scoreLabels" :key="key" justify="space-between" align="center">
            <n-text>{{ label }}</n-text>
            <n-progress :percentage="drafts.scoreResult[key]" :show-indicator="false" style="width: 200px" />
            <n-text strong>{{ drafts.scoreResult[key] }}</n-text>
          </n-space>
        </n-space>
        <n-divider />
        <n-card v-if="drafts.scoreResult.comment" size="small" :bordered="false">
          <n-text depth="3" style="font-size: 13px">{{ drafts.scoreResult.comment }}</n-text>
        </n-card>
      </n-space>
      <n-empty v-else description="暂无评分数据" />
      <template #footer>
        <n-space justify="end">
          <n-button @click="drafts.showScoreModal = false">关闭</n-button>
        </n-space>
      </template>
    </n-modal>
  </div>
...
async function handleGeneratePrompt() {
  ...
  await prompt.generatePreview(editor.currentChapter.id, editor.selectedProfileId)
}
